...
2. **MetaVie about page** — HTML scraped, text extracted via Python HTMLParser
3. **GitHub**: github.com/peterpan42388
4. **Websites**: metavie.co, world.metavie.co
5. **Photos**:
   - `leo_01.JPG` → avatar (coffee shop working photo, very effective)
   - `leo_life.png` → sidebar (teaching/life collage photo)
   - `leo_education.png` → original sidebar (replaced in iteration)
...
- 2025.08: Founded MetaVie Ltd (UK registered)
- Human Design algorithm reverse-engineered from NASA JPL ephemeris data
- OpenClaw desktop installer: 0 CLI, for non-technical users
- EOW (ELO Open World): multi-agent social framework, Cloudflare edge
...
- OpenClaw installer is standard Step 02 for EOW onboarding

### From GitHub search (peterpan42388 + openclaw ecosystem)
- OpenClaw: personal AI assistant, any OS/platform, "the lobster way 🦞"
...
| 3 | Agent产品落地 | OpenClaw installer + EOW + MetaVie platform |
...
|----|------|-------|-----------------|
| P1 | OpenClaw 桌面安装器 | #0A66C2 blue | 0-CLI, standard EOW Step 02 |
| P2 | EOW 多智能体框架 | #00a8cc cyan | Live, Agent Motivation Bridge, self-deploy |
| P3 | MetaVie 人格平台 | #6366f1 indigo | NASA JPL algorithm, global rarity |
...
- Changed avatar from `leo_life.png` to `leo_01.JPG` (better professional look)
- Changed sidebar photo from `leo_education.png` to `leo_life.png` (more personal/warm)
...
- `何驱之_AI-Agent-Engineer_视觉优化版_2026.html` — primary deliverable
- `何驱之_AI-Agent-Engineer_视觉优化版_2026.pdf` — trimmed via pypdf
